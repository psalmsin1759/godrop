...
export function customerPasswordResetEmail(opts: {
  firstName: string;
  email: string;
  resetLink: string;
}): EmailOptions {
  const html = emailLayout(
    cardHeader("Reset Your Password", "#f97316") +
    cardBody(`
      <p style="margin:0 0 16px;font-size:15px;color:#374151;">Hi <strong>${opts.firstName}</strong>,</p>
      <p style="margin:0 0 16px;font-size:14px;color:#374151;line-height:1.6;">
        We received a request to reset the password for your Godrop account
