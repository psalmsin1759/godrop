      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-full max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-xl bg-white p-6 shadow-xl focus:outline-none">
          <div className="flex items-start gap-4">
            <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0" style={{ backgroundColor: '#FFE3E1' }}>
              <AlertTriangle className="w-5 h-5" style={{ color: '#FF3B30' }} />
            </div>
            <div className="flex-1 min-w-0">
              <Dialog.Title className="text-sm font-bold text-[#0D1426]">{title}</Dialog.Title>
              <Dialog.Description className="mt-1 text-xs text-[#525A72] leading-relaxed">
                {description}
              </Dialog.Description>
            </div>
            <Dialog.Close className="w-6 h-6 flex items-center justify-center rounded hover:bg-[#EDF0F6] shrink-0">
