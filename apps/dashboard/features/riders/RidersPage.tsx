...
const kycConfig: Record<RiderKycStatus, { bg: string; text: string; label: string }> = {
  ...
  SUBMITTED: { bg: '#E7EEFF', text: '#1E5FFF', label: 'Submitted' },
  VERIFIED:  { bg: '#DFF5EC', text: '#1DB980', label: 'Verified' },
...
function RiderOrdersTab({ id }: { id: string }) {
  ...
  const orderStatusColors: Record<string, { bg: string; text: string }> = {
    DELIVERED:      { bg: '#DFF5EC', text: '#1DB980' },
    IN_TRANSIT:     { bg: '#E7EEFF', text: '#1E5FFF' },
    PICKED_UP:      { bg: '#E7EEFF', text: '#1E5FFF' },
    READY_FOR_PICKUP: { bg: '#FBEDD7', text: '#E8930C' },
    ACCEPTED:       { bg: '#FBEDD7', text: '#E8930C' },
    PENDING:        { bg: '#EDF0F6', text: '#525A72' },
    CANCELLED:      { bg: '#FFE3E1', text: '#FF3B30' },
    FAILED:         { bg: '#FFE3E1', text: '#FF3B30' },
    PREPARING:      { bg: '#FBEDD7', text: '#E8930C' },
  }

  if (isLoading) {
    return <div className="flex items-center justify-center py-10"><Loader2 className="w-4 h-4 animate-spin text-[#1E5FFF]" /></div>
  }
    ...
    <div className="flex flex-col">
      {orders.length === 0 ? (
        <div className="flex flex-col items-center gap-2 py-10">
          <Package className="w-7 h-7 text-[#DDE2EC]" />
          <p className="text-xs text-[#9AA1B4]">No orders yet.</p>
        </div>
      ) : (
        <div className="divide-y divide-[#F7F9FC]">
          {orders.map((order) => {
            const sc = orderStatusColors[order.status] ?? { bg: '#EDF0F6', text: '#525A72' }
            return (
              <div key={order.id} className="px-5 py-3 flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-xs font-mono font-semibold text-[#0D1426]">{order.trackingCode}</span>
                    <span className="text-[10px] font-medium rounded-full px-2 py-0.5" style={{ backgroundColor: sc.bg, color: sc.text }}>
                      {order.status.replace(/_/g, ' ')}
                    </span>
                  </div>
                  <p className="text-[11px] text-[#525A72] truncate">{order.pickupAddress} → {order.dropoffAddress}</p>
                ...
                <div className="text-right shrink-0">
                  <p className="text-xs font-semibold text-[#0D1426]">{formatNaira(order.deliveryFeeKobo)}</p>
                  <p className="text-[10px] text-[#9AA1B4]">{order.type}</p>
                </div>
              </div>
            )
          })}
      ...
      {meta && meta.totalPages > 1 && (
        <div className="px-5 py-3 border-t border-[#EDF0F6] flex items-center justify-between">
          <p className="text-[11px] text-[#9AA1B4]">Page {page} of {meta.totalPages}</p>
          <div className="flex items-center gap-1">
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1}
              className="w-6 h-6 rounded border border-[#E7EAF1] flex items-center justify-center disabled:opacity-40 hover:bg-[#F7F9FC]">
              <ChevronLeft className="w-3 h-3 text-[#525A72]" />
            </button>
            <button onClick={() => setPage((p) => Math.min(meta.totalPages, p + 1))} disabled={page >= meta.totalPages}
              className="w-6 h-6 rounded border border-[#E7EAF1] flex items-center justify-center disabled:opacity-40 hover:bg-[#F7F9FC]">
              <ChevronRight className="w-3 h-3 text-[#525A72]" />
            </button>
          </div>
        </div>
      )}
...
function RiderEarningsTab({ id }: { id: string }) {
  ...
  return (
    <div className="flex flex-col">
      ...
      {earnings.length === 0 ? (
        <div className="flex flex-col items-center gap-2 py-10">
          <Wallet className="w-7 h-7 text-[#DDE2EC]" />
          <p className="text-xs text-[#9AA1B4]">No earnings yet.</p>
        </div>
      ) : (
        <div className="divide-y divide-[#F7F9FC]">
          {earnings.map((earning) => (
            <div key={earning.id} className="px-5 py-3 flex items-center justify-between gap-3">
              <div className="flex-1 min-w-0">
                <p className="text-xs font-mono text-[#0D1426] font-medium">{earning.order.trackingCode}</p>
                <p className="text-[11px] text-[#525A72] truncate mt-0.5">
    ...
    <div className="fixed inset-0 z-50 flex" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#EDF0F6] sticky top-0 bg-white z-10">
          <h2 className="text-sm font-bold text-[#0D1426]">Rider Detail</h2>
          <button
            onClick={onClose}
            className="w-7 h-7 rounded flex items-center justify-center hover:bg-[#EDF0F6]"
          >
        ...
        {isLoading ? (
          ...
          <div className="flex-1 flex items-center justify-center text-xs text-[#FF3B30]">
            Failed to load rider.
          </div>
        ) : (
          <>
            {/* Tabs */}
            <div className="flex items-center gap-0 border-b border-[#EDF0F6] px-5 sticky top-[57px] bg-white z-10">
              {(['profile', 'orders', 'earnings'] as DrawerTab[]).map((t) => (
                <button
                  ...
                  style={tab === t
                    ? { color: '#1E5FFF', borderColor: '#1E5FFF' }
                    : { color: '#9AA1B4', borderColor: 'transparent' }}
                >
                  {t}
                </button>
              ))}
...
function Field({ label, value }: { label: string; value?: string | number | null }) {
  ...
  return (
    <div>
      <p className="text-[10px] text-[#9AA1B4] uppercase tracking-wide font-medium">{label}</p>
      <p className="text-xs text-[#0D1426] mt-0.5 font-medium">{value}</p>
    </div>
  )
}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="px-5 py-4 border-b border-[#F7F9FC]">
      <p className="text-[11px] font-bold text-[#1E5FFF] uppercase tracking-wider mb-3">{title}</p>
...
function RiderDetailContent({ rider }: { rider: RiderDetail }) {
  ...
  return (
    <>
      {/* Avatar + name */}
      <div className="px-5 py-5 flex items-center gap-4 border-b border-[#EDF0F6]">
        <div
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-[#0D1426]">{rider.firstName} {rider.lastName}</p>
          <p className="text-xs text-[#9AA1B4] font-mono">{rider.phone}</p>
          <div className="flex items-center gap-2 mt-1">
            ...
            <span
              ...
              style={rider.isActive ? { backgroundColor: '#DFF5EC', color: '#1DB980' } : { backgroundColor: '#EDF0F6', color: '#525A72' }}
            >
              {rider.isActive ? 'Active' : 'Inactive'}
            </span>
            {rider.isAvailable && (
              <span className="text-[11px] font-medium rounded-full px-2 py-0.5" style={{ backgroundColor: '#E7EEFF', color: '#1E5FFF' }}>
                Available
              </span>
            )}
          </div>
      ...
      {rider.guarantors && rider.guarantors.length > 0 && (
        <div className="px-5 py-4 border-b border-[#F7F9FC]">
          <p className="text-[11px] font-bold text-[#1E5FFF] uppercase tracking-wider mb-3">Guarantors</p>
          <div className="space-y-3">
            {rider.guarantors.map((g, i) => (
              <div key={i} className="bg-[#F7F9FC] rounded-lg p-3 space-y-1.5">
                <p className="text-xs font-semibold text-[#0D1426]">{g.name}</p>
                <p className="text-[11px] text-[#525A72] font-mono">{g.phone}</p>
                <p className="text-[11px] text-[#9AA1B4]">{g.address}</p>
                {g.governmentIdUrl && (
                  <a
                    href={g.governmentIdUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1 text-[11px] font-semibold text-[#1E5FFF] hover:underline mt-0.5"
                  >
      ...
      {/* Documents */}
      <div className="px-5 py-4 border-b border-[#F7F9FC]">
        <p className="text-[11px] font-bold text-[#1E5FFF] uppercase tracking-wider mb-3">
        ...
        <div className="space-y-2">
          ...
          <div
            ...
            style={{
              borderColor: rider.documents?.governmentIdUrl ? '#1E5FFF' : '#E7EAF1',
              backgroundColor: rider.documents?.governmentIdUrl ? '#E7EEFF' : '#F7F9FC',
            }}
          >
            <div
              className="w-7 h-7 rounded flex items-center justify-center shrink-0"
              style={{ background: rider.documents?.governmentIdUrl ? '#1E5FFF' : '#E7EAF1' }}
            >
              <FileText className="w-3.5 h-3.5" style={{ color: rider.documents?.governmentIdUrl ? '#fff' : '#9AA1B4' }} />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[11px] font-semibold text-[#0D1426]">Government-Issued ID</p>
              {rider.documents?.governmentIdUrl ? (
                <a
                  href={rider.documents.governmentIdUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 text-[10px] font-semibold text-[#1E5FFF] hover:underline"
                >
                  View <ExternalLink className="w-2.5 h-2.5" />
                </a>
              ) : (
                <p className="text-[10px] text-[#FF3B30] font-medium">Not uploaded</p>
              )}
            </div>
          </div>
          ...
          {rider.documents?.vehiclePaperUrls && rider.documents.vehiclePaperUrls.length > 0 ? (
            rider.documents.vehiclePaperUrls.map((url, i) => (
              <div
                key={i}
                className="flex items-center gap-3 p-2.5 rounded-lg border"
                style={{ borderColor: '#1E5FFF', backgroundColor: '#E7EEFF' }}
              >
                <div className="w-7 h-7 rounded flex items-center justify-center shrink-0 bg-[#1E5FFF]">
                  <FileText className="w-3.5 h-3.5 text-white" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[11px] font-semibold text-[#0D1426]">Vehicle Paper {i + 1}</p>
                  <a
                    href={url}
                    target="_blank"
                    rel="noopener noreferrer"
            ...
            <div
              className="flex items-center gap-3 p-2.5 rounded-lg border border-[#E7EAF1] bg-[#F7F9FC]"
            >
              <div className="w-7 h-7 rounded flex items-center justify-center shrink-0 bg-[#E7EAF1]">
                <FileText className="w-3.5 h-3.5 text-[#9AA1B4]" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[11px] font-semibold text-[#0D1426]">Vehicle Papers</p>
                <p className="text-[10px] text-[#FF3B30] font-medium">Not uploaded</p>
              </div>
            </div>
          )}
...
function RiderActionMenu({ rider, onClose, onView }: { rider: Rider; onClose: () => void; onView: () => void }) {
  ...
  if (step === 'reject_kyc') {
    return (
      <div className="absolute right-0 top-8 z-50 w-64 bg-white rounded-lg border border-[#E7EAF1] shadow-card-md p-3 space-y-2">
        <p className="text-xs font-semibold text-[#0D1426]">Reject KYC</p>
        <textarea
          value={notes}
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="flex-1 text-xs py-1.5 rounded text-white font-medium disabled:opacity-50 bg-[#FF3B30]"
          >
  ...
  return (
    <div className="absolute right-0 top-8 z-50 w-44 bg-white rounded-lg border border-[#E7EAF1] shadow-card-md py-1">
      ...
      >
        <Eye className="w-3.5 h-3.5 text-[#1E5FFF]" /> View detail
      </button>
      ...
      <div className="border-t border-[#EDF0F6] mt-1">
        <button
          ...
          disabled={loading}
          className="flex items-center gap-2 w-full px-3 py-2 text-xs hover:bg-[#F7F9FC]"
          style={{ color: rider.isActive ? '#FF3B30' : '#1DB980' }}
        >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#EDF0F6]">
          <h2 className="text-sm font-bold text-[#0D1426]">Add Rider</h2>
          ...
          <div className="grid grid-cols-2 gap-3">
            {([
              ...
              <div key={field} className={field === 'phone' ? 'col-span-2 sm:col-span-1' : ''}>
                <label className="block text-[11px] font-medium text-[#525A72] mb-1">{label}</label>
                <input
                  type={field === 'email' ? 'email' : 'text'}
                  value={(form[field] as string) ?? ''}
                  onChange={(e) => set(field, e.target.value)}
                  className="w-full text-xs rounded border border-[#E7EAF1] bg-[#F7F9FC] px-3 py-1.5 text-[#0D1426] focus:outline-none focus:ring-1 focus:ring-[#1E5FFF]"
                />
              </div>
            ))}
            <div>
              <label className="block text-[11px] font-medium text-[#525A72] mb-1">Vehicle type</label>
              <select
            ...
            <div>
              <label className="block text-[11px] font-medium text-[#525A72] mb-1">Plate number</label>
              <input
                type="text"
...
function RiderStatsBar() {
  ...
  const cards = [
    { label: 'Total Riders', value: stats?.total ?? '—', icon: <Users className="w-4 h-4 text-[#1E5FFF]" />, bg: '#E7EEFF' },
    { label: 'Active', value: stats?.active ?? '—', icon: <TrendingUp className="w-4 h-4 text-[#1DB980]" />, bg: '#DFF5EC' },
    { label: 'Online Now', value: stats?.available ?? '—', icon: <Bike className="w-4 h-4 text-[#E8930C]" />, bg: '#FBEDD7' },
    { label: 'KYC Pending', value: (stats?.byKycStatus.PENDING ?? 0) + (stats?.byKycStatus.SUBMITTED ?? 0), icon: <Package className="w-4 h-4 text-[#FF3B30]" />, bg: '#FFE3E1' },
  ]
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
      {cards.map((c) => (
        <div key={c.label} className="card p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-[11px] text-[#9AA1B4] font-medium">{c.label}</p>
            <p className="text-lg font-bold text-[#0D1426] leading-tight">{c.value}</p>
          </div>
        </div>
      ))}
    </div>
                    ...
                    <tr>
                      <td colSpan={9} className="text-center py-10">
                        <div className="flex flex-col items-center gap-2">
                          <Bike className="w-8 h-8 text-[#DDE2EC]" />
                          <p className="text-xs text-[#9AA1B4]">No riders found.</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                        ...
                        >
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2.5">
                              ...
                              <div>
                                ...
                                {rider.email && (
                                  <p className="text-[11px] text-[#9AA1B4]">{rider.email}</p>
                                )}
                              </div>
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            <span className="text-xs text-[#525A72] font-mono">{rider.phone}</span>
                          </td>
                          <td className="px-4 py-3">
                            ...
                            {rider.vehiclePlate && (
                              <p className="text-[11px] text-[#9AA1B4] font-mono">{rider.vehiclePlate}</p>
                            )}
                          </td>
                          <td className="px-4 py-3">
                            <span
                          ...
                          <td className="px-4 py-3">
                            <div className="flex flex-col gap-0.5">
                              ...
                              {rider.isAvailable && (
                                <span className="text-[10px] text-[#1E5FFF]">● Online</span>
                              )}
                            </div>
                          </td>
                          <td className="px-4 py-3">
