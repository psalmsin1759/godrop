# CLAUDE.md — Godrop Customer App (Flutter)

## What this app is
The customer-facing Flutter app for Godrop in Nigeria.
Customers use it to place food, grocery, retail, and parcel delivery orders, book trucks, track orders live, and manage their wallet.
...
- **Navigation**: go_router (`lib/app/customer_app.dart`)
...
lib/
  main_customer.dart
  app/
    customer_app.dart   # MaterialApp + go_router setup
    theme.dart          # Godrop brand colors and typography
  features/
    auth/               # Login, register, OTP verification
      bloc/             # AuthBloc / AuthCubit
    home/               # Category selector (food, grocery, retail, parcel, truck)
      bloc/
    orders/             # Order placement, cart, checkout
      bloc/
    tracking/           # Live order tracking with map
      bloc/
    truck/              # Truck booking flow
      bloc/
...
```bash
flutter pub get
flutter run -t lib/main_customer.dart         # on connected device or emulator
flutter build apk -t lib/main_customer.dart   # release APK
```
