...
/// Modern bottom sheet for picking a pickup or drop-off location —
/// surfaces the user's current location, saved places (Home/Work/Other),
/// popular Lagos areas, and live search in one friendly flow.
...
class _LocationPickerSheetState extends State<LocationPickerSheet> {
  ...
  void _selectSaved(SavedAddress a) {
    Navigator.of(context)
        .pop(ParcelLocation(lat: a.lat, lng: a.lng, name: a.address));
  }
  ...
  Future<void> _selectPopularArea(String name) async {
    ...
    final loc = await PlacesService.getDetails(predictions.first.placeId, name);
    if (!mounted) return;
    if (loc == null) {
      setState(() => _resolving = false);
      return;
      ...
      children: [
        if (widget.showCurrentLocation)
          _PickerRow(
            ...
            subtitle: 'Pin your live GPS position',
            onTap: _useCurrentLocation,
          ),
        if (saved.isNotEmpty) ...[
          const _PickerSectionLabel('Saved places'),
          ...saved.map((a) => _PickerRow(
                icon: _iconForLabel(a.label),
                iconGradient: _gradientForLabel(a.label),
                title: a.label,
                subtitle: a.address,
  ...
  static LinearGradient _gradientForLabel(String label) {
    switch (label.toLowerCase()) {
      case 'home':
        return GodropColors.blueGradient;
      case 'work':
        return GodropColors.orangeGradient;
      default:
        return GodropColors.blueGradient;
    }
  }
