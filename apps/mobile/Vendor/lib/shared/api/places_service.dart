...
/// A resolved place with coordinates, returned by the location picker.
class PickedPlace {
  final double lat;
  final double lng;
  final String name;

  const PickedPlace({required this.lat, required this.lng, required this.name});
}
...
/// Google Places autocomplete/details — same key and endpoints as the
/// customer app's places_service.dart (keep in sync).
class PlacesService {
  ...
  static Future<PickedPlace?> getDetails(String placeId, String name) async {
    try {
